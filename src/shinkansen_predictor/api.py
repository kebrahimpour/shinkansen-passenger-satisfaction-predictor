...
class JourneyData(BaseModel):
    ...
    service_class: str = Field(
        ..., 
        description="Service class: Ordinary, Green, or GranClass",
        example="Green"
    )
    on_time_performance: Optional[float] = Field(
        0.95,
        description="On-time performance ratio (0-1)",
        ge=0,
        le=1,
        example=0.95
    )
    weather_condition: Optional[str] = Field(
        "clear",
        description="Weather condition during journey",
        example="clear"
    )
    seat_occupancy: Optional[float] = Field(
        0.8,
        description="Seat occupancy ratio (0-1)",
        ge=0,
        le=1,
        example=0.8
    )
...
class PredictionResponse(BaseModel):
    ...
    satisfaction_score: float = Field(
        ...,
        description="Predicted satisfaction score (0-5)",
        ge=0,
        le=5,
        example=4.2
    )
...
app = FastAPI(
    ...
    contact={
        ...
        "url": "https://github.com/kebrahimpour/shinkansen-passenger-satisfaction-predictor",
    },
    license_info={
        "name": "CC0-1.0",
...
async def load_model():
    ...
    try:
        ...
        X_train = [
            {'duration': 120, 'service_class': 'Green', 'on_time_performance': 0.95, 'weather_condition': 'clear', 'seat_occupancy': 0.8},
            {'duration': 90, 'service_class': 'Ordinary', 'on_time_performance': 0.88, 'weather_condition': 'rain', 'seat_occupancy': 0.6},
            {'duration': 180, 'service_class': 'GranClass', 'on_time_performance': 0.99, 'weather_condition': 'clear', 'seat_occupancy': 0.9},
            {'duration': 60, 'service_class': 'Ordinary', 'on_time_performance': 0.92, 'weather_condition': 'cloudy', 'seat_occupancy': 0.7}
        ]
