...
class TestSatisfactionPredictor:
    ...
    def setup_method(self):
        ...
        self.sample_X = [
            {'duration': 120, 'service_class': 'Green', 'on_time_performance': 0.95, 
             'weather_condition': 'clear', 'seat_occupancy': 0.8},
            {'duration': 90, 'service_class': 'Ordinary', 'on_time_performance': 0.88, 
             'weather_condition': 'rain', 'seat_occupancy': 0.6},
            {'duration': 180, 'service_class': 'GranClass', 'on_time_performance': 0.99, 
             'weather_condition': 'clear', 'seat_occupancy': 0.9},
            {'duration': 60, 'service_class': 'Ordinary', 'on_time_performance': 0.92, 
             'weather_condition': 'cloudy', 'seat_occupancy': 0.7}
        ]
    ...
    def test_predict_basic(self):
        ...
        test_data = {'duration': 100, 'service_class': 'Green', 
                    'on_time_performance': 0.93, 'weather_condition': 'clear', 
                    'seat_occupancy': 0.75}
    ...
    def test_predict_invalid_service_class(self):
        ...
        invalid_data = {
            'duration': 100, 'service_class': 'InvalidClass',
            'on_time_performance': 0.93, 'weather_condition': 'clear',
            'seat_occupancy': 0.75
        }
    ...
    def test_prediction_bounds(self):
        ...
        edge_cases = [
            {'duration': 1, 'service_class': 'Ordinary', 'on_time_performance': 0.0,
             'weather_condition': 'storm', 'seat_occupancy': 1.0},
            {'duration': 1000, 'service_class': 'GranClass', 'on_time_performance': 1.0,
             'weather_condition': 'clear', 'seat_occupancy': 0.1},
        ]
        
        for case in edge_cases:
            prediction = self.predictor.predict(case)
            assert 0 <= prediction <= 5, f"Prediction {prediction} out of bounds for case {case}"
    ...
    def test_model_attributes_after_fit(self):
        ...
        assert not hasattr(self.predictor, '_is_fitted') or not self.predictor._is_fitted
    ...
    def test_multiple_fits_overwrite(self):
        ...
        new_X = [{'duration': 200, 'service_class': 'Green', 'on_time_performance': 0.95,
                  'weather_condition': 'clear', 'seat_occupancy': 0.8}]
        new_y = [4.0]
